...
export default function shareViewPlugin(context: LoadContext, options: { title?: string } = {}): Plugin<void> {
  return {
    name: 'share-view-plugin',
    async postBuild({ outDir }) {
      const title = options.title ?? (await siteTitle(context.siteDir, context.siteConfig.title));
